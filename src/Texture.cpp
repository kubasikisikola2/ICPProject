...
GLuint Texture::ckboard_ = 0;

void Texture::init() {
    gen_ckboard();
    std::cout << "initialized default checkerboard texture with id: " << ckboard_ << std::endl;
}
...
void Texture::gen_ckboard(void) {
    if (glIsTexture(Texture::ckboard_) != GL_TRUE) { // default checker-board texture yet not valid texture
