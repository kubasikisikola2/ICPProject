...
void Player::set_mode(PlayerMode newMode){
    ...
    void Player::update(GLFWwindow* window, float dt, Camera& camera){
        ...
        if (mode == PlayerMode::FreeFly){
            ...
            position.x = std::clamp(position.x, room_min_x + collision_radius, room_max_x - collision_radius);
            position.z = std::clamp(position.z, room_min_z + collision_radius, room_max_z - collision_radius);
        }


        camera.setPosition(get_eye_position());
}